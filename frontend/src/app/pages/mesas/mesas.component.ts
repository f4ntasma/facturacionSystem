...
export class MesasComponent implements OnInit {
  ...
  cargarMesas() {
    this.http.get<Mesa[]>(this.apiUrl).subscribe({
      next: (mesas) => {
        this.mesas = mesas;
  ...
  agregarMesa() {
    ...
    this.http.post<Mesa>(this.apiUrl, body).subscribe({
  ...
  guardarCambiosMesa() {
    ...
    this.http.put<Mesa>(`${this.apiUrl}/${this.mesaEditando.id}`, body).subscribe({
      next: () => {
  ...
  cambiarEstado(id: string, estado: string) {
    this.http.patch(`${this.apiUrl}/${id}/estado?estado=${estado}`, {}).subscribe({
      next: () => {
        this.openDropdownId = null;
        this.cargarMesas();
      },
