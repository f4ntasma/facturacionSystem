...
export interface Venta {
  ...
  menuAbierto?: boolean;
  estado?: string;
  facturaId?: string;  // comprobante electronico vinculado (null = sin emitir)
}
...
export class VentasComponent implements OnInit {
  ...
  convertirOrdenAVenta(orden: Orden): Venta {
    // createdAt viene como string ISO-8601 desde el backend (como por ejemplo: "2025-01-15T10:30:00Z")
  ...
  emitirComprobante(venta: Venta) {
    // Abre el wizard de comprobantes prellenado con los datos de esta venta
    this.router.navigate(['/app/facturas/nuevo'], { queryParams: { ordenId: venta.id } });
  }
