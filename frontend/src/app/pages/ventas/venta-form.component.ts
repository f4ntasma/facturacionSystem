...
export class VentaFormComponent implements OnInit {
  ...
  buscarDni() {
    ...
    this.http.get<any>(`${environment.apiUrl}/consulta/dni/${dni}`).subscribe({
      next: (res) => {
  ...
  async guardarVenta() {
    ...
    if (this.ventaForm.tipoComprobante === 'FACTURA' && !/^\d{11}$/.test(this.ventaForm.clienteRuc)) {
    ...
    try {
      ...
      const checkoutRequest: CheckoutRequest = {
        ...
        clienteRuc: this.ventaForm.clienteRuc || undefined
      };
